  ...
  const page_nation_link = useCallback(() => {
    ...
    for (let a = 0; a < page_array.length; a++) {
      for (let b = a + 1; b < link_array.length; b++) {
        if (page_array[a] === link_array[b]) {
          //좌우 이동버튼 클릭은 useEffect로 감지가 되서 배열에 잘 들어가지만 숫자 이동클릭은 안들어가기때문에 클릭했을때 같다면 splice가 되면 안되니깐 막기
  ...
  return (
    <React.Fragment>
      <div className="number_btn_wrap">
        <p onClick={page_nation_link} ref={ele_page_index}>
          {link_index}
        </p>
      </div>
    </React.Fragment>
