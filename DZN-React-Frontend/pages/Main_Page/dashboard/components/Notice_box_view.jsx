    ...
    <React.Fragment>
      {boxes_length === 0 ? (
        ...
        <div className="new_small_white_box">
          <Link to={`/support/notice/content/${boxes_lists[index].ntc_idx}`}>
            <Card className="news_white_card">
              <div className="card_text_box">
                <h2 className="card_text_hover">{boxes_lists[index].ntc_type}</h2>
                <p className="card_small_text_hover">
                  {textLengthOverCut(boxes_lists[index].ntc_title)}
                </p>
                <p className="card_footer_text">
                  WEHAGO Developers
                  <br />
                  {moment(boxes_lists[index].regDt).format("YYYY.MM.DD")}
                </p>
              </div>
            </Card>
          </Link>
