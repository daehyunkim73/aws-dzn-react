...
const Infra_service_using = () => {
  const { dataSave, setInfraLogic, infraLogic } = useServiceSalesContext_sale();
  ...
  useEffect(() => {
    (async function () {
      try {
        const infra_apply_list = await Server_ajax_get(
          `svccenter/infra_apply_list`
        );
        setInfra_posts(infra_apply_list);
      } catch (e) {
  ...
  useEffect(() => {
    ...
    (async function () {
      try {
        const infra_req_list = await Server_ajax_post(
          `svccenter/infra_req_list`,
          body
        );
        setInfra_reqres(infra_req_list);
        setInfraLogic(false);
      } catch (e) {
        return console.error(e);
      }
