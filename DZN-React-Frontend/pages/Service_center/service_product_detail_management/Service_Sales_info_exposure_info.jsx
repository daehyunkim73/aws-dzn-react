...
const Service_Sales_info_exposure_info = () => {
  ...
  const Exposure_Click = useCallback(() => {
    if (show_gbn !== "Y") {
      // dataSave[0].stat 값 1 = 제작중, 2 = 심사중, 4 = 심사 반려
      if (
        dataSave[0].stat !== 1 &&
        dataSave[0].stat !== 2 &&
        dataSave[0].stat !== 4
  ...
  return (
    <React.Fragment>
      ...
      <Exposure_popup_v2 setShow_gbn_svc={setShow_gbn_svc} />{" "}
          ...
          <div className="checkbox_radio_double_wrap">
            <div className="exposure_info_input_right_wrap">
              <select className="form-control" onChange={cateSettingEvt}>
                ...
                {svcCateState === true &&
                  svcSaleCateList[1].map((val, cnt) => {
                    return svc_cate === val.sub_cate_name ? (
                      <option value={val.sub_cate_code} selected key={cnt}>
                        {val.sub_cate_name}
                      </option>
                    ) : (
                      <option value={val.sub_cate_code} key={cnt}>
                        {val.sub_cate_name}
                      </option>
                    );
                  })}
