...
const Service_Sales_info_payment_info = () => {
  ...
  const payment_cate_click_evt = (e) => {
    ...
    if (e.target.id === "license_none_check") {
      // 요금제 방식 선택 시 0을 저장
      setSvcPaidPaymentGbn(0);
    } else {
      // 라이선스 방식 선택 시 1을 저장
      setSvcPaidPaymentGbn(1);
    }
  };
  ...
  useEffect(() => {
    if (svcSaleArrayLogic === true) {
      try {
        (async function () {
          try {
            const result = await Server_ajax_post(
              "service_center_managment/getSvcFeeInfo",
              {
                pdsvc_idx: serviceID,
              }
            );
    ...
    <React.Fragment>
      <div className="Sales_info_payment_info_wrap clearfix">
        <div className="Sales_info_payment_info_first_wrap">
          <div
            className={svcPayType === 1 && "Service_Sales_info_payment_method"}
          >
            <p>
              ...
              <div className="Sales_info_payment_info_input_wrap">
                ...
                <div className="radio_inputButton_box radio_inputButton_box_btn">
                  <input
                    ...
                    checked={svcPaidPaymentGbn === 0 && true}
                ...
                <div className="radio_inputButton_box radio_inputButton_box_btn">
                  <input
                    ...
                    name="license_check"
                    id="license_check"
                    checked={svcPaidPaymentGbn === 1 && true}
                    onClick={payment_cate_click_evt}
                  />
                ...
                {svcSaleArrayLogic === true &&
                  [].map.call(svc_pay_lic_array, (item, idx) => {
                    return (
                      <div
                        key={idx}
                        className="Sales_info_payment_info_input Sales_info_payment_info_input_text_wrap"
                      >
                        <p className="nbsp"></p>
                        <img
                          src={large_list_icon}
                        ...
                        <input
                          ...
                          ref={(el) =>
                            (svc_pay_lic_price_ref.current[idx] = el)
                          }
                          onChange={handleFeeInputData}
                        />
                        <span>원</span>
